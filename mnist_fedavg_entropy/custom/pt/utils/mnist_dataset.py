...
class MNIST_Idx(torch.utils.data.Dataset):
    def __init__(self, root, data_idx=None, train=True, transform=None, download=False):
        ...
        self.data, self.target = self.__build_mnist_subset__()

    def __build_mnist_subset__(self):
        # if index provided, extract subset, otherwise use the whole set
        mnist_dataobj = datasets.MNIST(self.root, self.train, self.transform, self.download)
